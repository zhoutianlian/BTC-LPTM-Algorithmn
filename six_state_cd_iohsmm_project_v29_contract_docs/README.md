...
- 从输入 `.zip` 或输入目录自动识别 CSV 文件；`.zip` 会先解压到输出目录下的临时目录。
...
```bash
cd /Users/zhoutianlian/Documents/MyGitHub/WTRCodeHub/BTC-LPTM-Algorithmn

pip install -r six_state_cd_iohsmm_project_v29_contract_docs/requirements.txt

python -m six_state_cd_iohsmm_project_v29_contract_docs.scripts.run_pipeline
```

运行路径统一配置在：

```text
six_state_cd_iohsmm_project_v29_contract_docs/configs/run_config.yaml
```

默认配置使用 `BTC-LPTM-FeatureEngineering/model_input/` 作为输入目录，输出到项目内的 `outputs/run_latest/`。如需切换输入 `.zip`、输入目录或输出目录，只改 `run_config.yaml` 即可。

仍然保留兼容命令行覆盖，例如 `--input-path`、`--output-dir`、`--config-dir` 和旧参数名 `--input-zip`。
...
## V2.6 Strict Input Contract

From V2.6 onward, the project uses a strict fixed input bundle. All CSV roles listed in `configs/feature_contract.yaml` are required, including `path_absorption_multiscale.csv`, `price_context_features.csv`, `liqprice_features.csv`, and `features_liq_dataflow.csv`. Earlier versions allowed several of these files to be optional during feature-engineering migration; this is no longer recommended because strategy-facing outputs must be reproducible and comparable across runs. See `docs/FEATURE_CONTRACT.md` for details.

## 文档入口

当前项目文档已合并精简。新读者请优先阅读：

```text
docs/ALGORITHM_PRINCIPLES.md
docs/ALGORITHM_PRINCIPLES.html
```

该文档以“先直觉、后技术”的方式解释模型要解决的问题、清算—价格反馈机制、六状态定义、belief vector、no-leakage、CD-IOHSMM-lite、directional quality、price expectation compliance 以及策略接入方式。
