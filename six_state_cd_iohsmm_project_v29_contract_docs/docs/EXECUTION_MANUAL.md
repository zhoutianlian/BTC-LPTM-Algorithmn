...
run:
  ...
  input_path: ../../BTC-LPTM-FeatureEngineering/model_input
  output_dir: outputs/run_latest
  config_dir: configs
```

其中 `input_path` 支持两种形式：

- `.zip` 文件：运行时自动解压并识别其中的 CSV。
- 目录：直接递归识别目录内的 CSV，例如 `model_input/`。

如果只想临时覆盖配置，也可以继续使用命令行参数：

```bash
python -m six_state_cd_iohsmm_project_v29_contract_docs.scripts.run_pipeline \
  --input-path /path/to/model_input \
  --output-dir /tmp/six_state_run
```

为了兼容旧脚本，`--input-zip` 仍然可用，并且现在同样接受目录。
...
## V2.6 Strict Input Bundle

The current version requires every CSV role in `configs/feature_contract.yaml`. In previous versions, some files were optional to support fallback paths during feature-engineering migration. That compatibility mode is intentionally removed from the current contract.

Required input files:

```text
base_context.csv
plie_predictions_source.csv or plie_source_features.csv
...
If any required file or required core column is missing, the pipeline should fail fast with a clear error. This behavior is preferred for production and research comparability: a missing feature should not silently change the state engine behavior.
