...
class CanonicalFrameBuilder:
    ...
    def build(self, paths: dict[str, Path | None]) -> tuple[pd.DataFrame, NoLeakageReport]:
        ...
        if price_ctx_path:
            ...
            notes.append("price_context_features.csv missing; strict contract should have failed before canonical construction.")

        # 7) Legacy liqprice proxy is now part of the fixed required input bundle.
        # It remains an auxiliary price/liquidation micro-context, while
        # price_context_features.csv is the primary price context layer.
        liqprice_path = paths.get("liqprice_features")
        if liqprice_path:
            liqprice = read_csv_normalized(liqprice_path)
            self._check_required("liqprice_features", liqprice, missing_required)
            ...
            notes.append("liqprice_features.csv missing; strict contract should have failed before canonical construction.")

        # 8) FHMV/liquidation stress auxiliary features are also part of
        # the fixed required input bundle. They are not allowed to dominate
        # the six-state model, but they provide a stable stress/volatility
        # diagnostic channel for auditing and auxiliary evidence.
        fhmv_path = paths.get("features_liq_dataflow")
        if fhmv_path:
            fhmv = read_csv_normalized(fhmv_path)
            self._check_required("features_liq_dataflow", fhmv, missing_required)
            fhmv = ensure_datetime(fhmv, ["time"])
            fhmv = self._dedup_time(fhmv, "features_liq_dataflow")
